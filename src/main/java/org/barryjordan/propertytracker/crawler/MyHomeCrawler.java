...
/**
 * Crawler implementation for MyHome property website.
 */
public class MyHomeCrawler extends Crawler {

    private static final Logger LOGGER = LoggerFactory.getLogger(MyHomeCrawler.class);
    private static final String BASE_URL ="http://www.myhome.ie";
    private static final String RENT_URL = BASE_URL + "/rentals/dublin-9/apartment-to-rent-in-santry?minbeds=2";
    private static final String SALE_URL = BASE_URL + "/residential/dublin-9/apartment-for-sale-in-santry?minbeds=2";
     ...
     */
    public MyHomeCrawler(final RestOperations restOperations) {
     ...
     * @param url The MyHome listing page URL.
    ...
    private void getListings(final List<PropertyVO> props, final String url, final PropertyType propertyType) {
        ...
        Elements propertyNodes = doc.select("li#resultItem");
        Element nextPageNode = doc.select("a.next").first();

        props.addAll(parseListing(propertyNodes, propertyType));

        if (nextPageNode != null) {
            String nextPath = nextPageNode.attr("href");
            String nextUrl = "";
            try {
                nextUrl = url.split("\\?")[0] + "?" + URLDecoder.decode(nextPath.substring(1), "UTF-8");
    ...
    private List<PropertyVO> parseListing(final Elements propertyNodes, final PropertyType propertyType) {
        ...
        for (Element propertyNode : propertyNodes) {
                ...
                Element fullAddressElem = (Element) propertyNode.childNode(1).childNode(1).childNode(5);
                String url = fullAddressElem.attr("href");
                String fullAddress = fullAddressElem.text();

                if (isMatch(fullAddress)) {
                    Element priceElem;
                    Element bedsElem;
                    if (propertyType.equals(PropertyType.RENT)) {
                        bedsElem = (Element) propertyNode.childNode(1).childNode(5).childNode(1);
                        priceElem = (Element) propertyNode.childNode(1).childNode(5).childNode(3);
                    } else {
                        bedsElem = (Element) propertyNode.childNode(1).childNode(5).childNode(0);
                        priceElem = (Element) propertyNode.childNode(1).childNode(5).childNode(1);
                    }
